...
fun main() {
    ...
    // dogs, more dogs than cats, are there are the same number of each.
        ...
        else-> println("There are same number of cats and dogs.")
        }
    ...
    val favorites = listOf("BigBoss","Avatar", "Freefire")
    println("List of favorites: $favorites")
    ...
    // Print out this new list.
        val leastFav = listOf("Rodies","Bang bang","PUBG")
        val combined = favorites + leastFav
    ...
    // this new list. (You may need to modify what you did for 10 and 11)
        ...
        val modify = combined.toMutableList()
        modify.removeAt(0)
        modify.removeAt(modify.size-1)
        println("Modifies list: $modify")
        


    // 13. Create a string variable and set it to null, then print the string
    var deepika : String? = null
                   println(deepika)
   ...
   val app = "FB"
  val size = when{
      app.isEmpty()->"The string is empty"
      app.length<5->"The string is short"
      app.length in 5..10 -> "The string is medium Length"
